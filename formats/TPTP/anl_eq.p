...
fof(e_in_rr,axiom, ![ X, Y] : ((dom(X) & dom(Y) & =(X,Y)) => (rr(X,Y)) )).
fof(r_in_rr,axiom, ![ X, Y] : ((r(X,Y)) => (rr(X,Y)) )).
fof(trans_rr,axiom, ![ X, Y, Z] : ((rr(X,Y) & rr(Y,Z)) => (rr(X,Z)) )).
fof(e_in_ss,axiom, ![ X, Y] : ((dom(X) & dom(Y) & =(X,Y)) => (ss(X,Y)) )).
...
fof(e_or_rrr,axiom, ![ X, Y] : ((rr(X,Y)) => ((=(X,Y))| ? [Z ]:  ((dom(Z) & r(X,Z) & rr(Z,Y)))) )).
fof(e_or_sss,axiom, ![ X, Y] : ((ss(X,Y)) => ((=(X,Y))| ? [Z ]:  ((dom(Z) & s(X,Z) & ss(Z,Y)))) )).
